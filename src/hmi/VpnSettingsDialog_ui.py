...
class Ui_VpnSettingsDialog(object):
    ...
    def retranslateUi(self, VpnSettingsDialog):
        ...
        self.autoConnectCB.setText(QCoreApplication.translate("VpnSettingsDialog", u"Route ALL ngPost connections through the VPN (override per-server)", None))
#if QT_CONFIG(tooltip)
        self.autoConnectCB.setToolTip(QCoreApplication.translate("VpnSettingsDialog", u"When checked: every NNTP connection from ngPost is forced through the VPN tunnel, regardless of each server's \"Use VPN\" flag. The VPN auto-starts when a job begins and disconnects 30 s after the queue is empty. Use this if you want a single global switch instead of fiddling with the per-server column.<br/><br/>When unchecked: only the servers whose \"Use VPN\" column is ticked route through the tunnel; the others connect directly.", None))
